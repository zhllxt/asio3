...
#endif
{
	...
	{
		auto operator()(auto state, auto ex_ref, request_option&& opt) -> void
		{
			...
			{
				stream = std::make_unique<asio::ssl::stream<asio::ip::tcp::socket&>>(sock, opt.sslctx);
			...
			if (stream)
			{
				...
				auto [e6, n6] = co_await http::async_read(*stream, buffer, resp, asio::use_nothrow_deferred);
				if (e6)
				{
					co_await stream->async_shutdown(asio::use_nothrow_deferred);
		...
		#endif
			{
				...
				beast::flat_buffer buffer{};
				auto [e6, n6] = co_await http::async_read(sock, buffer, resp, asio::use_nothrow_deferred);
				if (e6)
					co_return{ e6, std::move(resp) };
			}
	...
	SendToken&& token = asio::default_token_type<asio::tcp_socket>())
{
	return asio::async_initiate<SendToken, void(asio::error_code, http::response<http::string_body>)>(
		asio::experimental::co_composed<void(asio::error_code, http::response<http::string_body>)>(
			detail::async_request_op{}, executor),
		token,
		std::cref(executor),
		std::move(opt));
}
