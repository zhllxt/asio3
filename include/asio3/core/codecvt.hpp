...
#    include <Windows.h>
#    ifndef ASIO3_LOCALE_USE_WIN32_API
#    define ASIO3_LOCALE_USE_WIN32_API
#    endif
#  endif
#endif
...
template <
    ...
    asio::codecvt_mode _Mymode = asio::codecvt_mode{}>
...
#endif
{
    ...
    #if !defined(BOOST_LOCALE_USE_WIN32_API) && !defined(BHO_LOCALE_USE_WIN32_API) && !defined(ASIO3_LOCALE_USE_WIN32_API)
	...
	inline auto gbk_to_utf8(const StringT& str, const std::string& locale_name = "chs") noexcept
	{
        ...
        // gbk to widechar
        {
            ...
            asio::wstring_convert<std::codecvt_byname<wchar_t, CharT, std::mbstate_t>, wchar_t, CharT> conv(c);
            try
            {
        ...
        // widechar to utf8
        {
            auto sv = asio::to_basic_string_view(w);

            asio::wstring_convert<asio::codecvt_utf8<wchar_t, CharT>, wchar_t, CharT> conv;
            try
            {
    ...
    inline auto utf8_to_gbk(const StringT& str, const std::string& locale_name = "chs") noexcept
	{
        ...
        // widechar to gbk
        {
            ...
            asio::wstring_convert<std::codecvt_byname<wchar_t, char, std::mbstate_t>> conv(c);
            try
            {
                return conv.to_bytes(sv.data(), sv.data() + sv.size());
            }
	...
	inline std::string utf8_to_locale(const StringT& str) noexcept
	{
        ...
        try
        {
            c = new std::codecvt_byname<wchar_t, char, std::mbstate_t>(asio::get_codecvt_locale());
        }
        catch (const std::exception&)
        {
            return std::string{};
	...
	inline std::string locale_to_utf8(const StringT& str) noexcept
	{
        ...
        try
        {
            c = new std::codecvt_byname<wchar_t, CharT, std::mbstate_t>(asio::get_codecvt_locale());
        }
        catch (const std::exception&)
        {
            return std::string{};
