...
#endif
{
	...
	requires is_char<CharT>
	inline ASIO_CONST_BUFFER buffer(CharT*& data) ASIO_NOEXCEPT
	{
	...
	template<class CharT, class Traits = std::char_traits<CharT>>
	requires is_char<CharT>
	inline ASIO_CONST_BUFFER buffer(const CharT*& data) ASIO_NOEXCEPT
	{
		return asio::buffer(std::basic_string_view<CharT>(data));
	}
	...
	template<class CharT, class Traits = std::char_traits<CharT>>
	requires is_char<CharT>
	inline ASIO_CONST_BUFFER buffer(CharT* const& data) ASIO_NOEXCEPT
	{
		return asio::buffer(std::basic_string_view<CharT>(data));
	}
	...
	template<class CharT, class Traits = std::char_traits<CharT>>
	requires is_char<CharT>
	inline ASIO_CONST_BUFFER buffer(const CharT* const& data) ASIO_NOEXCEPT
	{
		return asio::buffer(std::basic_string_view<CharT>(data));
	}
