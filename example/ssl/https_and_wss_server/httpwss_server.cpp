...
#include <asio3/http/httpwss_server.hpp>
#include "../certs.hpp"
...
net::awaitable<void> do_http_recv(net::httpwss_server& server, std::shared_ptr<net::https_session> session)
{
...
net::awaitable<void> http_client_join(net::httpwss_server& server, std::shared_ptr<net::https_session> session)
{
...
net::awaitable<void> start_server(net::httpwss_server& server, std::string listen_address, std::uint16_t listen_port)
{
...
int main()
{
	...
	std::filesystem::path root = std::filesystem::current_path(); // /asio3/build/example/ssl/https_and_wss_server
